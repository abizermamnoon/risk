...
class Board(object):
    ...
    def create(cls, n_players):
        ...
        Returns:
            Board: A board with territories randomly allocated to
            the players.
        """
    ...
    def is_valid_attack_path(self, path):
        ...
        a player's armies cannot move through territories
        they already occupy;
        they must move through enemy territories.
        All valid attacks, therefore, will follow a path
        of starting on one player's territory and moving
        trough enemy territories.
    ...
    def cost_of_attack_path(self, path):
        ...
        Args:
            path ([int]): a list of territory_ids which must be
            a valid attack path
    ...
    def can_fortify(self, source, target):
        '''
        At the end of a turn, a player may choose to fortify
        a target territory by moving armies from a source territory.
    ...
    def cheapest_attack_path(self, source, target):
        ...
        a cheapest valid attack path between two territories if
        such a path exists.
        There may be multiple valid cheapest attack paths (in
        which case it doesn't matter which this function returns),
        or there may be no valid attack paths (in which casei
    ...
    def continent(self, continent_id):
        ...
        Returns:
            generator: Generator of Territories.
        """
        return (t for t in self.data if t.territory_id in
                risk.definitions.continent_territories[continent_id])
    ...
    def n_continents(self, player_id):
        ...
        """
        return len([continent_id for continent_id in range(6) if
                    self.owns_continent(player_id, continent_id)])
    ...
    def owns_continent(self, player_id, continent_id):
        ...
        """
        return all((t.player_id == player_id for t
    ...
    def num_foreign_continent_territories(self, continent_id, player_id):
        ...
        """
        return sum(1 if t.player_id != player_id else 0 for t
                   in self.continent(continent_id))
    ...
    def possible_attacks(self, player_id):
        ...
        return [Move(from_t.territory_id, from_t.armies,
                ...
                for from_t in self.mobile(player_id) for to_t in
                self.hostile_neighbors(from_t.territory_id)]
    ...
    def possible_fortifications(self, player_id):
        ...
        return [Move(from_t.territory_id, from_t.armies,
                to_t.territory_id, to_t.player_id, to_t.armies)
                for from_t in self.mobile(player_id) for
                to_t in self.friendly_neighbors(from_t.territory_id)]
    ...
    def fortify(self, from_territory, to_territory, n_armies):
        ...
        if n_armies < 0 or self.armies(from_territory) <= n_armies:
            raise ValueError('Board: Cannot move {n} armies from \
                              territory {tid}.'
                             .format(n=n_armies, tid=from_territory))
        if to_territory not in [t.territory_id for t in
                                self.friendly_neighbors(from_territory)]:
            raise ValueError('Board: Cannot fortify, territories do \
                              not share owner and/or border.')
    ...
    def attack(self, from_territory, to_territory, attackers):
        ...
        if attackers < 1 or self.armies(from_territory) <= attackers:
            raise ValueError('Board: Cannot attack with {n} \
    ...
    def plot_board(self, path=None, plot_graph=False, filename=None):
        ...
        def plot_path(xs):
            ...
            for i, x in enumerate(xs[1:]):
                if (xs[i] == 19 and xs[i+1] == 1) or (xs[i] == 1
                                                      and xs[i+1] == 19):
    ...
    def fight(cls, attackers, defenders):
        ...
        wins = [att_d > def_d for att_d, def_d in zip(attack_dices,
                                                      defend_dices)]
        return len([w for w in wins if w is False]), len([w for w in wins
                                                          if w is True])
    ...
    def set_armies(self, territory_id, n):
        ...
        """
        if n < 1:
            raise ValueError(f"Board: cannot set the number of \
                          armies to <1 ({territory_id}, {n}).")
